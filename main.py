...
def parseSongs(songString):
    songs = [
        song.strip().removeprefix("-").strip()
        for song in songString.split("\n")
...
if autofill:
        ...
        if num_songs > 0:
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Generate {num_songs} songs."},
                ],
            )
    ...
    while True:
        ...
        for song in parsed_bot_response:
            ...
            if (
                user_add_playlist not in ["n", "N", "no", "No", "NO", ""]
                and user_add_playlist.isdigit()
                and int(user_add_playlist) <= len(search_result["tracks"]["items"])
            ):
